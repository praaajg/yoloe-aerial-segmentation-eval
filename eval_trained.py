...
CHECKPOINT = sys.argv[1] if len(sys.argv) > 1 else 'checkpoints/linear_probe_30ep.pt'
...
for start_mb in range(0, total, MB):
    ...
    for result, mask_path in zip(model.predict(mb_imgs, conf=CONF, retina_masks=False, verbose=False, batch=BATCH, max_det=100, stream=True), mb_masks):
...
results_dict = {
    ...
    'config': {'checkpoint': CHECKPOINT, 'conf': CONF, 'retina_masks': False, 'test_crops': total},
}

out_fn = 'test_' + Path(CHECKPOINT).stem + '_eval_metrics.json'
...
name_map = {0: 'background', 1: 'road', 2: 'water', 3: 'forest'}
for cls_id in range(NUM_CLASSES):
    print(f'  {name_map[cls_id]}: IoU={per_class_iou[cls_id]:.4f}')
print(f'Saved results/{out_fn}')
